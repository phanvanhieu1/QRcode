...
export class QrCodeService {
  ...
  isqrCodeExist = async (tableNumber: string) => {
    const table = await this.userService.check(tableNumber);
    return !!table;
  };
  ...
  async create(data: CreateQrCodeDto) {
    ...
    if (isExist === true) {
      throw new BadRequestException(
        `bàn: ${tableNumber} đã có, vui lòng kiểm tra lại`,
      );
    }
  ...
  isqrCodeExist2 = async (_id: string) => {
    const table = await this.userService.checkUser(_id);
    return table;
  };
  ...
  async saveQr(tableNumber: string, code: string) {
    ...
    if (!checkTable) {
      throw new BadRequestException(
        `không tìm thấy bàn số ${tableNumber}, vui lòng tạo bàn số ${tableNumber} trước`,
      );
    } else if (rs) {
      throw new BadRequestException(
        `bàn số ${tableNumber} đã có mã qr, vui lòng thử lại`,
      );
    }
